...
export default function EmployeeManagement() {
  ...
  const [newEmp, setNewEmp] = useState({ fullName: "", username: "", password: "", phone: "", role: "agent" });
  const [isSubmitting, setIsSubmitting] = useState(false);

  // Fetch Data
  const fetchEmployees = async () => {
    // setLoading(true); // Silent refresh এর জন্য লোডিং অফ রাখা যেতে পারে
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredEmployees.map((emp) => (
            ...
            >
              <div className={`w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold overflow-hidden border-2 ${emp.role === 'supervisor' ? 'border-orange-100 bg-orange-50 text-orange-600' : 'border-blue-100 bg-blue-50 text-blue-600'}`}>
                {emp.avatar_url ? (
              ...
              <div>
                ...
                <span className={`text-xs px-2 py-0.5 rounded-md mt-1 inline-block capitalize ${emp.role === 'supervisor' ? 'bg-orange-100 text-orange-700' : 'bg-blue-100 text-blue-700'}`}>
                   {emp.role}
                </span>
              </div>
            </motion.div>
             ...
             >
                ...
                <form onSubmit={handleCreate} className="space-y-4">
                   <div>
                      ...
                      <input required className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" 
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div>
                         ...
                         <input required placeholder="e.g. agent1" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" 
                            value={newEmp.username} onChange={e => setNewEmp({...newEmp, username: e.target.value.replace(/\s+/g, '').toLowerCase()})} />
                      ...
                      <div>
                         ...
                         <input required type="tel" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" 
                   ...
                   <div className="grid grid-cols-2 gap-4">
                      <div>
                         ...
                         <select className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500"
                      ...
                      <div>
                         ...
                         <input required type="password" className="w-full p-3 bg-gray-50 border border-gray-200 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" 
                            value={newEmp.password} onChange={e => setNewEmp({...newEmp, password: e.target.value})} />
                      </div>
                   </div>

                   <button disabled={isSubmitting} type="submit" className="w-full py-4 mt-4 bg-gray-900 text-white font-bold rounded-xl hover:bg-black transition-all flex justify-center shadow-lg">
